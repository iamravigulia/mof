    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      -->
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative -mx-8" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <a onclick="closemodalMOF({{$message}})" class="p-2 w-8 h-8 bg-gray-600 text-white rounded-full absolute right-0 -top-10 -mr-2 -mt-2 z-50" href="javascript:void(0);" style="z-index: 999999999999999999;">x</a>
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <form action="{{route('fmt.mof.update', $que->id)}}" method="post" enctype="multipart/form-data">
                    ...
                    @endforeach
                    {{-- add new options --}}
                    {{-- <div class="flex flex-wrap my-4" id="new1">
                        <div class="w-1/2 px-2">
                            <label class="w-full" for="">Answer</label>
                            <input class="my-2 border border-gray-500 p-2 w-full rounded-lg" type="text" name="new1" placeholder="Answer">
                        </div>
                        <div class="w-1/2 px-2">
                            <label class="w-full" for="">Arrange</label>
                            <input class="my-2 border border-gray-500 p-2 w-full rounded-lg" type="text" name="new_arrange_1" placeholder="Arrange">
                        </div>
                    </div>
                    <div class="flex flex-wrap my-4" id="new2">
                        <div class="w-1/2 px-2">
                            <label class="w-full" for="">Answer</label>
                            <input class="my-2 border border-gray-500 p-2 w-full rounded-lg" type="text" name="new2" placeholder="Answer">
                        </div>
                        <div class="w-1/2 px-2">
                            <label class="w-full" for="">Arrange</label>
                            <input class="my-2 border border-gray-500 p-2 w-full rounded-lg" type="text" name="new_arrange_2" placeholder="Arrange">
                        </div>
                    </div> --}}
                    <div class="w-full px-4">{{-- w-1/3 --}}
                        <div class="my-2">
